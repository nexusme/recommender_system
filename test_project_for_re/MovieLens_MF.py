...
def load_data():
    ...
    users1 = pd.read_csv('files_are_not_here/ml-100k/u.user', sep='|',
                         names=['user_id', 'age', 'sex', 'occupation', 'zip_code'], encoding='latin-1')

    ratings1 = pd.read_csv('files_are_not_here/ml-100k/u.data', sep='\t',
                           names=['user_id', 'movie_id', 'rating', 'unix_timestamp'], encoding='latin-1')

    # The movies file contains a binary feature for each genre.
    movies1 = pd.read_csv(
        'files_are_not_here/ml-100k/u.item', sep='|', names=[
            'movie_id', 'title', 'release_date', "video_release_date", "imdb_url", "genre_unknown", "Action",
    ...
    # 使id从0开始
    users1["user_id"] = users1["user_id"].apply(lambda x: str(x - 1))
    movies1["movie_id"] = movies1["movie_id"].apply(lambda x: str(x - 1))
    movies1["year"] = movies1['release_date'].apply(lambda x: str(x).split('-')[-1])
    ratings1["movie_id"] = ratings1["movie_id"].apply(lambda x: str(x - 1))
    ratings1["user_id"] = ratings1["user_id"].apply(lambda x: str(x - 1))
    ratings1["rating"] = ratings1["rating"].apply(lambda x: float(x))
    ...
    # 计算每个流派的电影数
    genre_occurences1 = movies1[genre_cols1].sum().to_dict()
    return users1, movies1, ratings1, genre_cols1, genre_occurences1
...
# 由于某些电影可以属于一种以上的流派，因此我们按以下方式创建不同的“流派”列：
# - all_genres: 电影的所有活跃流派
# - genre: 从活跃类型中随机抽样
...
def mark_genres(movies, genres):
    ...
    movielens1 = ratings.merge(movies, on='movie_id').merge(users, on='user_id')
    return movielens1


# 以下功能用于生成交互式Altair图表。 我们将显示由给定属性切片的数据直方图，并创建用于切片数据的过滤器。
# A function that generates a histogram of filtered data.
def create_filter():
    # Create filters to be used to slice the data.
    occupation_filter1 = alt.selection_multi(fields=["occupation"])
    occupation_chart1 = alt.Chart().mark_bar().encode(
        x="count()",
        ...
        color=alt.condition(
            ...
            alt.value("lightgray")),
    ).properties(width=300, height=300, selection=occupation_filter1)
    return occupation_filter1, occupation_chart1
...
def save_plot_occupation(name):
    occupation_filter, occupation_chart = create_filter()
    users_ratings = (
        ratings
    ...
    user_occupation = alt.hconcat(
        filtered_hist('rating count', '# ratings / user', occupation_filter),
        filtered_hist('rating mean', 'mean user rating', occupation_filter),
        occupation_chart,
        data=users_ratings)
    user_occupation.save(name)
...
def sparse_mean_square_error(sparse_ratings, user_embeddings, movie_embeddings):
    # sparse_ratings: A SparseTensor rating matrix N*M
    # user_embeddings: A dense Tensor U N*k where k is the embedding dimension, U_i is the embedding of user i.
    # movie_embeddings: A dense Tensor V of shape M * k,  k is the embedding dimension, V_j is the embedding of movie j.
    predictions = tf.reduce_sum(  # 求para的和 axis=1, 转为1维张量
        tf.gather(user_embeddings, sparse_ratings.indices[:, 0]) *  # 用一个一维的索引数组，将张量中对应索引的向量提取出来
        tf.gather(movie_embeddings, sparse_ratings.indices[:, 1]), axis=1)
...
def build_model(ratings, embedding_dim=3, init_stddev=1.):
    ...
    U = tf.Variable(tf.random_normal(  # 用于初始化变量, 函数用于从“服从指定正态分布的序列”中随机取出指定个数的值。
...
if __name__ == '__main__':
    ...
    users, movies, ratings, genre_cols, genre_occurences = load_data()
    movielens = mark_genres(movies, genre_cols)
    ...
    # save_plot_occupation('files_are_not_here/ml_100K_plots/user_occupation.html')
    # save_plot_movies('files_are_not_here/ml_100K_plots/movies.html')
    ...
    # movie_embedding_norm(model).save('files_are_not_here/ml_100K_plots/movie_embedding_norm.html')
    ...
    '''嵌入可视化 T分布随机邻居嵌入'''
    # tsne_movie_embeddings(model_lowinit).save('files_are_not_here/ml_100K_plots/T-distributed.html')
    ...
    #
    # chart = movie_embedding_norm([model, model_lowinit, reg_model])
    #  alt.Chart.serve(chart)
    #
    # tsne = tsne_movie_embeddings(reg_model)
    # alt.Chart.serve(tsne)
    ...
    embedding_norm = movie_embedding_norm(softmax_model)
    alt.Chart.serve(embedding_norm)
    tsne_movie = tsne_movie_embeddings(softmax_model)
    alt.Chart.serve(tsne_movie)
